<script>
  ...
  // SVELTE 5: Reaktivní stav pomocí runy $state
  ...
  const rightNav = navItems.slice(2, 4); // Opraven rozsah pro 4 položky
...
<div class="mobile-menu {isMenuOpen ? 'open' : ''}">
  <button class="close-btn" onclick={toggleMenu} aria-label="Zavřít menu">
...
<style>
  ...
  .site-header::after {
    ...
    position: absolute;
    top: 0;
    right: calc(var(--header-radius) * -1);
    width: var(--header-radius);
    height: var(--header-radius);
    background: radial-gradient(
      circle at 100% 100%, 
  ...
  .desktop-nav a:hover, .desktop-nav a.active { color: #16a34a; }
  ...
  .mobile-links a { 
    ...
    color: #000; border-bottom: 1px solid #f0f0f0;
